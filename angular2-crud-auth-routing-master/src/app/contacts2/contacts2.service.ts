...
export class ContactsService {
  ...
  contactsChanged = new EventEmitter<Observable<Contact2[]>>();

  private url: string = 'app/contacts2';

  constructor(private http: Http) { }

  getAll(): Observable<Contact2[]> {
  ...
  get(id){
    return this.getAll()
      .map((list: any) => list.find(contact2 => contact2.id == id))
      .catch(this.handleError);
  }
